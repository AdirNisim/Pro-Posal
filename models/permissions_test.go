...
func testPermissionToOneCompanyUsingCompany(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadCompany(ctx, tx, false, (*[]*Permission)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testPermissionToOneSetOpCompanyUsingCompany(t *testing.T) {
	var err error
...
var (
	permissionDBTypes = map[string]string{`ID`: `uuid`, `UserID`: `uuid`, `CompanyID`: `uuid`, `Role`: `text`, `ContractID`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_                 = bytes.MinRead
)
