...
func testCategoryToOneCompanyUsingCompany(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadCompany(ctx, tx, false, (*[]*Category)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCategoryToOneSetOpCompanyUsingCompany(t *testing.T) {
	var err error
...
var (
	categoryDBTypes = map[string]string{`ID`: `uuid`, `CompanyID`: `uuid`, `CategoryID`: `uuid`, `Description`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_               = bytes.MinRead
)
