...
func testUserToManyContactCompanies(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.ContactCompanies().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if v.ContactID == b.ContactID {
			bFound = true
		}
		if v.ContactID == c.ContactID {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadContactCompanies(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ContactCompanies); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ContactCompanies = nil
	if err = a.L.LoadContactCompanies(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ContactCompanies); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyCreatedByOffers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.CreatedByOffers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadCreatedByOffers(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CreatedByOffers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.CreatedByOffers = nil
	if err = a.L.LoadCreatedByOffers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CreatedByOffers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyCustomerOffers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.CustomerOffers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadCustomerOffers(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CustomerOffers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.CustomerOffers = nil
	if err = a.L.LoadCustomerOffers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CustomerOffers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpContactCompanies(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddContactCompanies(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ContactCompanies[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ContactCompanies[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ContactCompanies().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyAddOpCreatedByOffers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddCreatedByOffers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.CreatedByOffers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.CreatedByOffers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.CreatedByOffers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManyAddOpCustomerOffers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddCustomerOffers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.CustomerOffers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.CustomerOffers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.CustomerOffers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `FirstName`: `text`, `LastName`: `text`, `Phone`: `text`, `Email`: `text`, `EmailHash`: `text`, `PasswordHash`: `text`, `InvitedBy`: `uuid`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
